...
{
    // 4 cuadrantes de la mitad de tamano.
    ...
    previas.swap(particulas_);
    for (Particula *p : previas)
    {
        if (noroeste_->insertar(p))
            continue;
        if (noreste_->insertar(p))
            continue;
        if (sudoeste_->insertar(p))
            continue;
        if (sudeste_->insertar(p))
            continue;
...
bool QuadTree::insertar(Particula *p)
{
    ...
    // 2) Si es una hoja con espacio (o ya llegamos a la profundidad maxima),la guardamos aqui. evita recursion infinita
    if (!dividido_ &&
