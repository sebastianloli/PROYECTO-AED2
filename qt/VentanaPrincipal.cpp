...
VentanaPrincipal::VentanaPrincipal(QWidget* parent) : QWidget(parent) {
    ...
    // ---------------- Controles de parametros configurables ----------------
    auto* spinN = new QSpinBox;            // numero de particulas
    spinN->setRange(10, 50000);
    spinN->setValue(1000);
    spinN->setSingleStep(100);

    auto* spinCap = new QSpinBox;          // capacidad por nodo del QuadTree
    spinCap->setRange(1, 64);
    spinCap->setValue(4);

    auto* spinRadio = new QDoubleSpinBox;  // radio de las particulas
    spinRadio->setRange(1.0, 20.0);
    spinRadio->setValue(3.0);
    spinRadio->setSingleStep(0.5);

    auto* spinVel = new QDoubleSpinBox;    // velocidad maxima
    spinVel->setRange(0.0, 10.0);
    spinVel->setValue(1.0);
    spinVel->setSingleStep(0.5);

    auto* comboDist = new QComboBox;       // distribucion inicial
    ...
    // El lienzo informa metricas y las mostramos en el panel.
